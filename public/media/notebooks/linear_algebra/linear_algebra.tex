...
Let \( T: \mathbb{R}^n \to \mathbb{R}^m \)\footnote{This notation means that \(T\) is a mapping from \(\mathbb{R} \text{ to }\mathbb{R}\)} be a linear transformation. The matrix \( A \in \mathbb{R}^{m \times n} \) representing \( T \) can be formed as follows:
